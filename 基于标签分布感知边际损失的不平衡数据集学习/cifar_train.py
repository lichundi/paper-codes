...
model_names = sorted(name for name in models.__dict__
    ...
    and callable(models.__dict__[name]))  # callable功能为判断返回对象是否可调用（即某种功能）。
...
# 定义主函数main()
def main():
    args = parser.parse_args() ## 使用函数parse_args()进行参数解析
    ...
    if args.seed is not None:
        ...
        cudnn.deterministic = True #设置这个 flag 可以让内置的 cuDNN 的 auto-tuner 自动寻找最适合当前配置的高效算法，来达到优化运行效率的问题。
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.gpu is not None:
        ...
        model = model.cuda(args.gpu)#cuda()用于将变量传输到GPU上，model.cuda将model复制到cuda上
    else:
        # DataParallel will divide and allocate batch_size to all available GPUs
        model = torch.nn.DataParallel(model).cuda() #批处理，多GPU默认用dataparallel使用在多块gpu上

    optimizer = torch.optim.SGD(model.parameters(), args.lr,
                                momentum=args.momentum,
                                weight_decay=args.weight_decay)# optimizer 使用 SGD + momentum  动量，默认设置为0.9 权值衰减，默认为1e-4 

    # optionally resume from a checkpoint 恢复模型
    if args.resume:
        if os.path.isfile(args.resume):                                #os.path.isfile判断返回是否为文件
            print("=> loading checkpoint '{}'".format(args.resume))
            checkpoint = torch.load(args.resume, map_location='cuda:0') #load一个save得对象
            args.start_epoch = checkpoint['epoch']                      #defaut=200
            best_acc1 = checkpoint['best_acc1']                         #best_acc1 = 0
            if args.gpu is not None:
                # best_acc1 may be from a checkpoint from a different GPU
                best_acc1 = best_acc1.to(args.gpu)
            model.load_state_dict(checkpoint['state_dict'])            #load_state_dict仅仅加载模型参数
    ...
    transform_train = transforms.Compose([       #将几个transforms 组合在一起
        ...
        transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)), #标准化normalize: - mean / std
    ])
    ...
    log_training = open(os.path.join(args.root_log, args.store_name, 'log_train.csv'), 'w')#os.path.join把目录和文件名合成一个路径
    ...
    for epoch in range(args.start_epoch, args.epochs):
        adjust_learning_rate(optimizer, epoch, args) #adjust_learning_rate自定义函数
        ...
        tf_writer.add_scalar('acc/test_top1_best', best_acc1, epoch) #writer.add_scalar将我们所需要的数据保存在文件里面供可视化使用。 这里是Scalar类型，所以使用writer.add_scalar()
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer): 
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        #每 步输出一次
        if i % args.print_freq == 0:   #default=
