...
- DBDA中设计了两个分支来捕捉HSI中包含的大量光谱和空间特征。另外，在这两个分支上分别应用信道注意块和空间注意块，使得DBDA能够对提取的特征映射进行改善和优化。
- 四个高光谱数据集上的一系列实验表明，该框架在训练样本明显不足的情况下，具有优于现有算法的性能。
## 1、引言 ##
- 谱带信息的过度冗余和有限的训练样本给HSI分类带来了巨大的挑战
- Lee等人。[27]提出了一个具有更深层次和更广泛网络的上下文deepCNN（CDCNN）。在[28]中，陈等。结合正则化设计了基于3D-CNN的特征提取模型。
- 较深的网络可以捕捉到更精细的特性，但要训练更深入的网络则更困难。残差网络（ResNet）[29]和稠密卷积网络（DenseNet）[30]的出现缓解了深层网络训练的困难。受ResNet的启发，Zhong等人。[31]提出了一种谱空间残差网络（SSRN），它在训练样本有限的情况下更有效。Wang等人。[32]将DenseNet引入到他们的快速密集谱空间卷积（FDSSC）算法中
- 为了优化特征的识别，采用注意机制对特征图进行细化.Ma等人。[34]提出了一种基于卷积块注意模块（CBAM）的双分支多注意机制网络（DBMA）[35]，并获得了最好的分类结果。
- 本文受最新DBMA算法和自适应自注意机制双注意网络（DANet）[44]的启发，设计了用于HSI分类的双分支双注意机制网络（DBDA）。该框架包含两个分支，即光谱分支和空间分支，分别捕获光谱和空间特征。采用通道注意机制和空间注意机制对特征地图进行细化。通过连接两个分支的输出，我们得到了融合的光谱空间特征。最后，利用softmax函数确定分类结果。
## 2、相关工作 ## 
...
- Figure 1 
- （1）具有BN层的第（m+1）个3D-CNN层的第i个输出
### 2.3 ResNet and DenseNet ###
- 通常，卷积层越多，网络性能越好。然而，层数过多可能会导致梯度消失和梯度爆炸的问题变得更糟。ResNet[29]和DenseNet[30]是解决该问题的有效方法。
- Figure 2a 在ResNet中的传统CNN模型中增加了一个跳转连接。如图2a所示，H表示隐藏块，它是一个包含卷积层、激活层和BN层的模块。跳转连接可以看作是单一映射，使输入数据能够直接通过网络传递。剩余块是ResNet中的基本单元，第l个剩余块的输出可以计算为：（3）
- Figure 2b 在ResNet的基础上，DenseNet直接连接各个层，以保证网络各层之间的信息流最大化。DenseNet不是像ResNet那样通过求和来组合特性，而是通过在通道维度中串联特性来组合特性。Dense块是DenseNet的基本单元，第l个Dense块的输出可以计算为：
- Figure 3 假设输入特征映射的形状为n个通道的p×p×b，每个卷积层由k个核组成，每个核的形状为1×1×d，则每层都生成k个通道的p×p×b的特征映射。然而，密集连接在通道维度连接特征映射，因此通道数量和卷积层数量之间存在线性关系。m层致密块体产生的km道输出可表示为：（5）
### 2.4 注意机制 ###
- 3D-CNN的一个缺点是所有的空间像素和谱带在空间域和谱域中都具有相等的权重。显然，不同的光谱波段和空间像素对特征提取的贡献是不同的。注意机制是解决这一问题的有力手段。受人类视觉感知过程的启发[46]，注意力机制被设计成更多地关注信息区域，而较少考虑非必要区域。
#### 2.4.1 光谱注意块 ####
...
- (10) HSI分类常用的定量指标用来衡量预测结果和真实值之间不同是交叉熵损失函数
### 3.1DBDA网络框架 ###
...
- 在空间分支和频谱分支的最后一个BN层和全局平均池层之间分别采用了一个dropout层，dropout是防止过拟合一种简单而有效的方法，通过在训练阶段去除给定百分比p的单元，使得其他单元的存在不可靠，这就阻碍了单元之间的协同适应。
- 早期停止策略和动态学习率调整方法。提前停止意味着如果损失函数在一定的时间段内不再减少（在我们的模型中是20个），那么我们将提前停止训练过程，以防止过度拟合并减少训练时间
- 学习速率是训练网络的一个重要超参数，动态学习速率可以帮助网络避免某些局部极小值。采用余弦退火[53]方法动态调整学习速率，如（13）式中，ηt是第i次运行中的学习速率，ηi min，ηi max表示学习率的范围
## 4、实验结果 ##
...
- ResNet。输入尺寸为5×5×b，其中b表示光谱带数。
- SSRN:SSRN的体系结构是在[31]中提出的，它是基于3D-CNN和ResNet的。输入的大小为7×7×b。
- FDSSC:FDSSC的体系结构见[32]，它基于3D-CNN和DenseNet。输入的大小为9×9×b。
- DBMA:DBMA的体系结构如[34]所示，它基于3D-CNN、DenseNet和一种注意力机制。7×7×b是输入块大小。
- 对于CDCNN、SSRN、FDSSC、DBMA和所提出的方法，batch size设置为16，优化器设置为Adam，学习率为0.0005。早期停止策略的上限设置为200个epochs。如果验证集中的损失在20个epochs内不再下降，那么我们将终止训练阶段。
### 4.3 分类图和分类结果 ###
#### 4.3.1 IP 数据结果 ####
- Table 7 具有3%训练样本的IP数据集的分类结果
- Figure 9 使用3%训练样本的IP数据集的分类图。（a） 假彩色图像。（b） GT。（c–h）不同算法的分类图。
#### 4.3.2 UP 数据结果 ####
- Table 8 具有0.5%训练样本的UP数据集的分类结果
- Figure 10 使用0.5%训练样本的UP数据集的分类图。（a） 假彩色图像。（b）GT。（c–h）不同算法的分类图。
#### 4.3.3 SV 数据结果 ####
- Table 9 具有0.5%训练样本的SV数据集的分类结果
- Figure 11 使用0.5%训练样本的SV数据集的分类图。（a） 假彩色图像。（b） GT。（c–h）不同算法的分类图。
#### 4.3.4 BS 数据结果 ####
- Table 10 具有1.2%训练样本的BS数据集的分类结果
- Figure 12 使用1.2%训练样本的BS数据集的分类图。（a） 假彩色图像。（b） GT。（c–h）不同算法的分类图。
### 4.4 运行时间 ###
...
- Table 11 on the IP dataset
- Table 12 on the UP dataset
- Table 13 on the SV dataset 
### 5、讨论 ### 
#### 5.1 训练样本比例 ####
- Figure 13
####  5.2.注意机制的有效性 ####
- Figure 14 注意机制的有效性（不同注意机制的结果）。
####  5.3.激活函数的有效性 ####
- Figure 15 激活函数的有效性（不同激活函数的结果）。
